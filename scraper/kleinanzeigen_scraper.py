...
class KleinanzeigenScraper:
    ...
    def filter_accept(self, title, price, qm_price) -> bool:
        '''
    ...
    def clean_rent_data(self, posting):
        ...
        posting_entry = {}
        if self.filter_accept(posting_ellipsis, posting_price, price_per_qm):
    ...
    def convert_rent_data_page_to_dictionary(self, page_strings):
        ...
        for page_string in page_strings:
            ...
            for posting in all_postings:
                try:
                    current_hash, posting_entry = self.clean_rent_data(posting)
    ...
    def request_data_for_specified_city(self, city_name, city_configuration):
        print(f"Requesting rental data for {city_name}!")
        print("please wait...")
        all_rent_data_pages = self.get_rent_data_pages(city_configuration["url"])
        print(f"Finished requesting for {city_name}")

        all_posting_dict_list = self.convert_rent_data_page_to_dictionary(all_rent_data_pages)
